...
// Prints crossword in this format:
// Orientation (Down or Across), Numbering (non-unique), Row, Column, Word (all caps)
void print_solution(char** crossword, Word** ord_words, int ** crossword_nums, int count) {
    for (int i = 0 ; i < count ; ++i) {
        Word* node = ord_words[i];
        if (node->orientation == Vertical) { // Vertical 
            printf("Down, %d, %d, %d, ", crossword_nums[node->begin][node->constant], node->begin, node->constant);
            for (int j = node->begin ; j <= node->end ; ++j) {
                putchar(crossword[j][node->constant]);
            }
            printf("\n");
        }
        else { // Horizontal 
            printf("Across, %d, %d, %d, ", crossword_nums[node->constant][node->begin], node->constant, node->begin);
            for (int j = node->begin ; j <= node->end ; ++j) {
                putchar(crossword[node->constant][j]);
            }
            printf("\n");
        }
    }
...
/**
 * if (node->orientation == Vertical) { // Vertical 
            printf("Down %d: {\n",crossword_nums[node->begin][node->constant]);
            printf("    clue: placeholder,\n");
            printf("    answer: \'");
            for (int j = node->begin ; j <= node->end ; ++j) {
                putchar(crossword[j][node->constant]);
            }
            ...
            printf(" Across %d: {\n",crossword_nums[node->constant][node->begin]);
            printf("    clue: placeholder,\n");
            printf("    answer: \'");
            for (int j = node->begin ; j <= node->end ; ++j) {
                putchar(crossword[node->constant][j]);
